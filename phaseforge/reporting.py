...
def figure_activation_map(figdir: Path, tab: Path) -> None:
    ...
    ax.text(
        ...
        "status at 150 °C is UNKNOWN without extra latency.",
...
def figure_diameter_map(figdir: Path) -> None:
    ...
    ax.set_title("Hinze/Grace characteristic diameter vs shear and IFT")
    ax.legend()
...
def figure_mechanics(figdir: Path) -> None:
    ...
    ax.set_title("Mechanical safety factor. 150 °C near typical pDCPD Tg is unresolved.")
    ax.legend()
    ax.set_ylim(0, 3.5)
...
def figure_permeability(figdir: Path, tab: Path) -> None:
    ...
    ax.set_yscale("log")
    ax.set_title("Residual permeability: distributed vs settled (Kozeny–Carman / dilute)")
    ax.legend()
...
def figure_feasible_window(figdir: Path, tab: Path, results: list[CoupledResult]) -> None:
    ...
    for r in results:
        ...
        ax.scatter(T, phi, c=colors[r.overall], s=48, edgecolors="k", linewidths=0.3)
    for lab, c in colors.items():
        ax.scatter([], [], c=c, label=lab.value, edgecolors="k")
    ax.set_xlabel("Temperature (°C)")
    ax.set_ylabel("Dispersed volume fraction φ")
    ax.set_title("Coupled feasibility (inhibitor index = 0.6, other parameters nominal)")
...
def figure_mc(figdir: Path, mc: dict[str, Any]) -> None:
    ...
    vals = [mc["probabilities"][k] for k in labels]
    ax.barh(labels, vals, color="#2b8cbe")
    ax.set_xlim(0, 1)
    ax.set_xlabel("Estimated probability of PASS (LHS, 40–90 °C envelope)")
...
def build_traceability(
    ...
    add(
        ...
        f"{nom.viscosity.mu_eff_cP:.2f} (nominal 60 C)",
        "cP",
    ...
    add(
        ...
        "kinetics + mechanics at T",
        "models evaluated to 150 C",
        "degC",
        RequirementStatus.UNKNOWN,
        "low at 150 C",
        "Madbouly 40-60 C; Hu 50-80 C; Kordes onset to 140 C (DSC, not isothermal 150 C)",
        "Isothermal latency and strength at 150 C.",
    )
    add(
    ...
    add(
        ...
        "mechanics",
        f"6000 psi = {PSI_6000_MPA:.3f} MPa",
        "psi",
        nom.mechanics.status,
        "medium at 60 C; low at 150 C",
        "PMC12566568; CHEN2023 crush at 52-69 MPa (epoxy analogue)",
    ...
    add(
        ...
        "Arrhenius t_solid",
        f"{nom.kinetics.t_transform_min:.1f} min nominal; HPHT {hpht.kinetics.t_transform_min:.2g} min",
        "min",
        nom.kinetics.status,
        "medium in 40-80 C; low/UNKNOWN at 150 C",
        "MADBOULY2025; HU2018",
    ...
    add(
        "R09_particle_size",
        "Hinze/Grace",
        f"{nom.droplets.d_particle_um:.0f}",
        "um",
        nom.droplets.status,
        "medium (order-of-magnitude; correlation uncertainty)",
    ...
    add(
        ...
        "coalescence risk score",
        f"{nom.droplets.coalescence_risk:.2f}",
        "risk 0-1",
        nom.agglomeration,
        "low",
    ...
    add(
        "R12_load_bearing",
        "mechanics SF",
        f"SF={nom.mechanics.SF:.2f} at nominal T",
        "-",
        nom.mechanics.status,
    ...
    add(
        ...
        "SF_mech vs 4500-6000 psi",
        f"SF={nom.mechanics.SF:.2f} (nom); HPHT SF={hpht.mechanics.SF:.2f}",
        "dimensionless",
        nom.mechanics.status if not nom.mechanics.unknown_high_T else RequirementStatus.UNKNOWN,
        "low at 150 C",
        "pDCPD Tg 140-165 C; Yang 95 MPa (T unspecified as 150 C)",
        "6000 psi crush at 150 C.",
    )
    add(
    ...
    add(
        ...
        f"porosity={nom.permeability.porosity:.2f}",
        "-",
        nom.permeability.status,
        "medium if discrete; FAIL if bulk gel",
        "Chen conductivity 10-30 MPa analogue",
        "Conductivity cell after in-situ formation.",
    )
    add(
        "R16_flow_through_around",
        "k_rel",
        f"{nom.permeability.k_rel:.3g}",
        "relative",
        nom.permeability.status,
        "medium",
        "Kozeny-Carman; Chen conductivity comparison",
...
def write_final_metrics(nom: CoupledResult, hpht: CoupledResult, mc: dict[str, Any], path: Path) -> None:
    payload = {
        ...
        "headline": {
            ...
            "answer_40_80C": nom.overall.value,
            "answer_150C": hpht.overall.value,
            "notes": (
                "A computationally plausible window exists at moderate temperature "
                "(approximately 45-70 C) with latent ROMP and φ~0.1-0.2. "
...
def generate_all_figures(*, skip_uq: bool = False) -> dict[str, Any]:
    ...
    pd.DataFrame([nom.as_dict(), hpht.as_dict(), hpht_assumed.as_dict()]).to_csv(
